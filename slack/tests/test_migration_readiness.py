#!/usr/bin/env python3
"""Harbor verifier entry point for the Acme migration readiness benchmark.
...
environment for a state export over ``admin.sock``, which is mode 0600 and
owned by the environment's own user, so the unprivileged agent could not have
produced or tampered with the answer this verifier reads.

The grade is the layered verifier's: ``verifiers`` supplies the framework,
``verifiers.contracts.acme_migration`` states what this task requires, and
...
from slack_sim.migration_reward import (  # noqa: E402
    IntegrityError, check_workspace_integrity,
)
from slack_sim.protocol import ADMIN_SOCKET, request  # noqa: E402
from verifiers import Episode, TieredRewardEngine, blank_reward_dict, reward_dict  # noqa: E402
from verifiers.contracts.acme_migration import build_contract  # noqa: E402
...
ADMIN = os.environ.get("SLACK_ADMIN_SOCKET", ADMIN_SOCKET)
...
def chosen_preset() -> str:
    ...
    An ablation is meant to be a configuration change rather than a fork, so
    the preset that graded a run is read from one place and written back into
    that run's own details.
    """
...
def evaluate(state: dict | None, preset: str | None = None) -> tuple[dict[str, float], dict]:
    ...
    try:
        # Seeded messages destroyed, events rewritten: the episode did not happen
        # in the world this task specifies, so no grader's number from it is
        # evidence about the agent.
        check_workspace_integrity(state)
    except IntegrityError as error:
        # The exception is the whole record of why this run was discarded, so
        # it is written down as data and not only as a sentence: `kind` says
        # which of four unrelated things went wrong, and the rest names the
        # sections, messages or events to go and look at. `integrity_error`
        # keeps its old string shape for anything already reading it.
        ...
        # A crash in the check is a defect in the grader, and must not be
        # filed under any of the kinds above -- those send someone to inspect
        # a workspace that may be perfectly intact.
...
def main() -> None:
    ...
    print(json.dumps({"rewards": rewards, "details": details}, sort_keys=True, indent=2, default=str))
