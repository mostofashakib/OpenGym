#!/usr/bin/env python3
"""JSON CLI for the GmailEnvironment contract."""
...
from gmail_sim.environment import DEFAULT_MAX_TURNS, GmailEnvironment
from gmail_sim.service import DEFAULT_DB_PATH, DEFAULT_SNAPSHOT_PATH
...
def main() -> None:
    ...
    parser.add_argument(
        "command",
        choices=["setup", "reset", "setup_state", "seed_session", "prompts", "render_prompt", "tools", "step", "state", "history"],
    )
    ...
    parser.add_argument("--prompt-id", default="gmail_assistant")
    ...
    environment = GmailEnvironment(args.db, args.snapshot)
    result: Any
    if args.command == "setup":
        ...
        result = environment.render_prompt(
            args.prompt_id,
            _instruction(args),
            json.loads(args.prompt_context),
        )
