...
The roles live in the environment (``task_sim.prompts.PromptInterface``), where
...
DEFAULT_PROMPT_ID = "handover_coordinator"

FALLBACK_SYSTEM = (
    "You are operating a task tracker through the tools you have been given. "
    "Inspect the workspace before you change anything, decide each record on "
    "its own fields, and leave every record the request does not name exactly "
    "as you found it. Read a record before replacing any list-valued field on "
    "it. Report what you changed when you are done."
)
...
def system_prompt(prompt_id: str = DEFAULT_PROMPT_ID, *, context: dict | None = None) -> str:
    ...
    try:
        from task_sim.prompts import get_prompt_interface
    except ImportError:
