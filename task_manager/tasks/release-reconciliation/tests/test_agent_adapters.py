...
class Sandbox:
    ...
    def reward(self) -> float:
        from verifiers.contracts.release_reconciliation import build_contract
...
CORRECT = [
    _call("list_tasks", {"project_id": "P005"}),
    _call("get_task", {"task_id": "TASK033"}),
    _call("get_task", {"task_id": "TASK035"}),
    _call("get_task", {"task_id": "TASK044"}),
    _call("get_task", {"task_id": "TASK048"}),
    _call("unlink_tasks", {"task_id": "TASK048", "depends_on_task_id": "TASK044"}),
    _call("link_tasks", {"task_id": "TASK048", "depends_on_task_id": "TASK041"}),
    _call("update_task", {"task_id": "TASK048", "status": "IN_PROGRESS"}),
    _call("update_task", {"task_id": "TASK048", "status": "COMPLETED"}),
    _call("update_task", {"task_id": "TASK044", "status": "IN_PROGRESS"}),
    _call("update_task", {"task_id": "TASK044", "status": "COMPLETED"}),
    _call("get_task", {"task_id": "TASK047"}),
    _call("update_task", {"task_id": "TASK035", "status": "IN_PROGRESS"}),
    _call("update_task", {"task_id": "TASK035", "status": "COMPLETED"}),
    _call(
        "update_task",
        {"task_id": "TASK042", "assignee": "U008", "priority": "URGENT", "status": "IN_PROGRESS"},
    ),
...
INCOMPLETE = [
    ...
    _call(
        "submit_handover_report",
        {
            "summary": "Partial inspection only.",
            "task_ids": ["TASK033"],
        },
    ),
    Completion(text="Done."),
...
def test_the_loop_actually_changes_the_world_it_is_graded_on() -> None:
    ...
    with Sandbox() as sandbox:
        asyncio.run(
            ToolLoop(ScriptedProvider(INCOMPLETE), sandbox.backend, system="s", max_turns=30).run("go")
        )
        reward = sandbox.reward()
        check("partial episode does not score 1.0", reward < 1.0, f"{reward:.4f}")
