...
reward -- that is the layered verifier's, in `verifiers.contracts.release_reconciliation`.
...
RELEASE_MILESTONES = tuple(event.event_id for event in RELEASE_RECONCILIATION_SCENARIO.events)
WORKSPACE_MILESTONES = RELEASE_MILESTONES
...
_RELEASE_EVENT_IDS = frozenset(e.event_id for e in RELEASE_RECONCILIATION_SCENARIO.events)
...
def _detect_scenario(state: dict[str, Any]) -> str:
    """Determine which scenario produced this state from its event ledger."""
    return "release_reconciliation"
...
def check_workspace_integrity(state: dict[str, Any]) -> None:
    ...
    expected_seeded = frozenset(row[0] for row in TRACKER_TASKS)
    vanished = expected_seeded - present
    if vanished:
        raise DestroyedHistoryError(vanished)

    declared = _RELEASE_EVENT_IDS
...
def side_effects(state: dict[str, Any]) -> list[str]:
    ...
    The target tasks for the release reconciliation scenario are exempt on
    assignee and labels.
    """
...
def _evaluate_workspace(state: dict[str, Any]) -> dict[str, bool]:
    ...
    target = [e["event_id"] for e in state.get("scenario_events", [])] or list(milestones)
...
__all__ = [
    ...
    "MissingSectionsError", "RELEASE_MILESTONES", "SEEDED_TASK_IDS",
