...
def _evaluate(
    ...
    if rule.trigger == "observed":
        has_table = connection.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='observed_ids'"
        ).fetchone()
        session_observed = {
            row[0] for row in connection.execute("SELECT observed_id FROM observed_ids")
        } if has_table else set()
        all_observed = action.observed | session_observed
        missing = [item for item in rule.observed_ids if item not in all_observed]
        ...
        predicates.append(
            Predicate(
                ...
                matched,
                rule.observed_ids,
                tuple(item for item in rule.observed_ids if item in all_observed),
            )
        )
        return matched, tuple(predicates)
...
def _record(connection: sqlite3.Connection, action: _Action) -> None:
    now = VIRTUAL_CLOCK.now(connection)
    ...
    if has_table:
        ...
        for obs_id in action.observed:
            connection.execute(
                "INSERT OR IGNORE INTO observed_ids (observed_id, first_seen_ms, first_seq) VALUES (?, ?, ?)",
                (obs_id, now, seq),
            )
