...
# trajectory, and that agent needs the same OpenRouter credentials `run.sh` gives
# the agent under test. Without them the inner agent reports "Not logged in",
# which surfaces as AgentAuthenticationError on the CLI and as a bare 500 from
# the viewer's /summarize endpoint.
#
# The model must also be a real OpenRouter id. Harbor's own default
# (`claude-haiku-4-5`) and the viewer's (`haiku`) are Anthropic tier aliases, and
# OpenRouter rejects both with "is not a valid model ID".
#
