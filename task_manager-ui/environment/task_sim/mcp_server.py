"""An MCP server that hands the task tracker to the agent's harness.
...
before the first turn. Nothing in the task prompt has to explain that a tracker
exists or how to drive it -- discovering the workspace is the harness's job, and
what is in the workspace is the agent's.
...
SERVER_INFO = {"name": "tasks", "version": "1.0.0"}
...
def tool_list() -> list[dict[str, Any]]:
    ...
    Generated rather than written down, so a tool added to the workspace reaches
    the agent without a second edit here -- and no tool can quietly go missing
    from the surface the agent is given.
    """
    return [
        {
            "name": tool["name"],
...
def _tool_result(envelope: dict[str, Any]) -> dict[str, Any]:
    ...
    told why it was denied can act on it; one handed an empty success learns
    something false about the workspace.

    Serialized compactly. Nothing reads this but a parser, and indentation was
    adding a third to every result the agent has to hold -- context spent on
    leading spaces rather than on what the workspace said.
    """
    return {
        "content": [
            {
...
def dispatch(message: dict[str, Any], socket_path: str = AGENT_SOCKET) -> dict[str, Any] | None:
    ...
    # `or {}` would fold every falsy non-object -- [], "", 0 -- into an empty
    # dict and wave it through, which is the one case this check exists for.
