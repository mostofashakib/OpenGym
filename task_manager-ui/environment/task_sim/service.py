...
SCENARIOS: dict[str, Scenario] = {
    "release_reconciliation": RELEASE_RECONCILIATION_SCENARIO,
}
...
def seed_database(
    ...
    with connect(db_path) as connection:
        ...
        dependencies = TRACKER_DEPENDENCIES
...
def link_tasks(
    ...
    with connect(db_path) as connection:
        ...
        dep_rows = connection.execute("SELECT dep_id FROM task_dependencies WHERE dep_id LIKE 'DEP%'").fetchall()
        max_num = max([int(r[0][3:]) for r in dep_rows if r[0][3:].isdigit()] or [0])
        dep_id = f"DEP{max_num + 1:03d}"
...
def main() -> None:
    ...
    parser.add_argument("--scenario", default="release_reconciliation", choices=list(SCENARIOS))
    ...
    if args.command == "seed":
        seed_database(db_path, snapshot_path, scenario)
        print(json.dumps({"seeded": True, "scenario": args.scenario}))
    elif args.command == "teardown":
        teardown_database(db_path, snapshot_path, scenario)
