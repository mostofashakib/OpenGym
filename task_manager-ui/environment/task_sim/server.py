...
class WorldService:
    ...
    def __init__(self, db_path: Path, snapshot_path: Path, scenario: "Scenario | None" = None) -> None:
        self.db_path = db_path
        self.snapshot_path = snapshot_path
        from task_sim.scenario import Scenario as _Scenario  # noqa: F811
        self.scenario: _Scenario = scenario or RELEASE_RECONCILIATION_SCENARIO
    ...
    def teardown(self) -> dict[str, Any]:
        with self._lock, storage_errors():
            teardown_database(self.db_path, self.snapshot_path, self.scenario)
...
def serve(
    ...
    # Resolve scenario: CLI arg > env var > default.
    name = scenario_name or os.environ.get("TASK_SCENARIO", "release_reconciliation")
    from task_sim.scenario import Scenario  # noqa: F811
    scenario: Scenario = SCENARIOS.get(name, RELEASE_RECONCILIATION_SCENARIO)
    world = WorldService(db_path, snapshot_path, scenario)
    ...
    LOGGER.info("tasksd listening on %s (agent) and %s (admin), scenario=%s", agent_socket, admin_socket, name)
...
def main() -> None:
    ...
    serve(Path(args.db), Path(args.snapshot), args.agent_socket, args.admin_socket, args.scenario)
