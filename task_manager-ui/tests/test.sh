...
selftest_status=0
for suite in test_virtual_clock test_scenario_engine test_task_surface test_mcp_server test_tool_contract test_rl_contract test_verifiers test_harbor_reward test_reward_validation test_reward_matrix test_integrity_violations test_agent_adapters test_environment_contract test_task_bridge; do
  echo "--- ${suite} ---"
...
export TASK_CONTRACT="release_reconciliation"
"$PYTHON_BIN" "${TESTS_DIR}/test_reassignment_readiness.py"
verifier_status=$?
