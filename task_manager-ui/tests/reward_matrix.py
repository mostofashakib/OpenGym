...
Deterministic, so the table comes out the same every time.
"""
...
ABLATIONS: tuple[tuple[str, tuple], ...] = (
    ...
    ("omit breaking circular deadlock", without("unlink_tasks")),
    ("omit compliance hold resolution", without("update_task", "TASK035")),
    ("omit the handover report", without("submit_handover_report")),
    ("report an incomplete set",
     tuple(("submit_handover_report", {"task_ids": ["TASK035", "TASK042"], "summary": "partial"})
           if tool == "submit_handover_report" else (tool, body)
           for tool, body in COMPETENT_RUN)),
    ("omit one deduplication", without("mark_task_duplicate", "TASK052")),
    ("omit two deduplications", without("mark_task_duplicate")),
    ...
    ("act as another user",
     COMPETENT_RUN + (("update_task", {"task_id": "TASK035", "actor_id": "U006"}),)),
    ("do nothing at all", ()),
)
