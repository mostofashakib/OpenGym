...
OMISSIONS = (
    "omit breaking circular deadlock",
    "omit compliance hold resolution",
    "omit the handover report",
    "report an incomplete set",
    "omit one deduplication",
    "omit two deduplications",
...
def test_the_decision_costs_more_than_the_routine() -> None:
    """Weight follows consequence: deadlock unlinking is high-leverage."""
    check("missing the deadlock fix costs more than missing one dedup move",
          SCORES["omit breaking circular deadlock"] < SCORES["omit one deduplication"],
          f"{SCORES['omit breaking circular deadlock']:.3f} vs "
          f"{SCORES['omit one deduplication']:.3f}")
    check("missing two dedup moves costs more than missing one",
          SCORES["omit two deduplications"] < SCORES["omit one deduplication"])
