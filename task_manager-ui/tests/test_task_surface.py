...
def test_the_seeded_workspace_is_what_the_task_describes() -> None:
    with Workspace() as workspace:
        ...
        for section, expected in (
            ("users", len(TRACKER_USERS)),
            ("projects", len(TRACKER_PROJECTS)),
            ("milestones", len(TRACKER_MILESTONES)),
            ("tasks", len(TRACKER_TASKS)),
            ("assignments", len(TRACKER_ASSIGNMENTS)),
            ("dependencies", len(TRACKER_DEPENDENCIES)),
        ):
