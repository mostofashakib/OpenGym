"""Harbor entry point.
    ...
    harbor run -p example_tasks/task_manager -a agent.harbor_agent:TrackerAgent \
...
``tasks`` client, over ``agent.sock``, as the ``agent`` user Harbor set.
"""
...
class TrackerAgent(BaseAgent):
    """Drive the tracker with whichever provider ``-m`` names.
    ...
    async def run(
        ...
        self.logger.info(
            "tracker-adapter: %s via %s", provider.spec, backend.describe()["backend"]
        )
        loop = ToolLoop(
            provider,
            backend,
        ...
        self.logger.info("tracker-adapter finished: %s", result.summary())
