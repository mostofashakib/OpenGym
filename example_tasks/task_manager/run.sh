...
# job containing config.json/result.json plus its trial directories. This is also
# the directory to pass to `harbor view`. Pointing the viewer at a single job
# directory makes it mistake trial result.json files for job results.
...
# `agent.harbor_agent:TrackerAgent` is an import path, and Harbor resolves it
