"""RL-facing lifecycle and state contract for the Slack incident world."""
...
from slack_sim.identity import LOGGED_IN_USER, SlackUserCredentials
from slack_sim.prompts import PromptInterface, get_prompt_interface
from slack_sim.scenario import Scenario
from slack_sim.seed import ACME_SCENARIO
from slack_sim.migration_reward import workspace_progress
...
from slack_sim.sqlite_common import connect, storage_errors
from slack_sim.tool_definitions import get_tool_definitions

CONTRACT_VERSION = "7.0"
DEFAULT_MAX_TURNS = 100
DEFAULT_PROMPT_ID = "slack_operator"
SESSION_COOKIE_PATTERN = re.compile(r"^[A-Za-z0-9._-]{1,128}$")


class SlackIncidentEnvironment:
    """
    Harbor supplies isolation at the container level. This adapter supplies the
    model-world contract within that container: lifecycle, prompt, tools,
    opaque session identity, tool transitions, and durable turn history.
    """
    ...
    def __init__(
        ...
        credentials: SlackUserCredentials = LOGGED_IN_USER,
        ...
        # Which task this environment is hosting. The simulator is the same
        # either way; hand it a different Scenario and it hosts a different one.
    ...
    def setup_state(self, seed: int = 0) -> None:
        ...
        if seed != 0:
            raise ValueError(
                f"scenario defines only deterministic seed 0, not {seed}"
            )
    ...
    def reset(
        ...
        *,
        ...
        return {
            ...
            "observation": {"workspace": "slack", "actor_id": self.credentials.user_id},
        }
    ...
    def seed_session(
        ...
        *,
        ...
        with connect(self.db_path) as connection:
            ...
            created_ts = VIRTUAL_CLOCK.now(connection)
            connection.execute(
                "DELETE FROM conversation_history WHERE session_cookie = ?",
                (session_cookie,),
            )
            connection.execute(
                """
                INSERT INTO rl_sessions (
                    session_cookie, actor_id, created_ts, turn_count,
            ...
            self._append_history(connection, session_cookie, 0, "system", "message", None, prompt["system"])
            self._append_history(connection, session_cookie, 0, "user", "message", None, user_instruction)
            connection.commit()
        return {
            "session_cookie": session_cookie,
            "actor_id": actor_id,
    ...
    def prompts(self) -> list[dict[str, str]]:
        """Return the prompt catalog exposed by this environment."""
    ...
    def step(
        ...
        action and then lost the database has still failed for a storage
        reason.
        """
        with storage_errors():
    ...
    def _step(
        ...
        Returns the observation plus the RL signals a trainer needs: `reward`
        is the change in progress caused by this action, `progress` is the
        absolute score so far, and `terminated`/`truncated` say whether the
        rollout is over by goal or by budget.
        """
        ...
        if int(session["finished"]):
            ...
            return {
                ...
                "terminated": bool(session["turn_count"] >= max_turns) is False,
        ...
        # Score the episode after the action. Only what the environment can
        # observe counts here -- the scenario events the agent's work released
        # and whether it posted an answer at all. Whether that answer is
        # *right* is the verifier's call: the verdict and the owners are short
        # strings a policy could assert without reading anything, so shaping on
        # them would reward the assertion rather than the work behind it.
        ...
        with connect(self.db_path) as connection:
            connection.execute(
                """
                UPDATE rl_sessions
                SET turn_count = ?, last_progress = ?, finished = ?
    ...
    def history(
        self, session_cookie: str, limit: int | None = None
    ) -> list[dict[str, Any]]:
        """Turn history, oldest first.

        `limit` returns only the most recent `limit` events, still in order.
        A long rollout's transcript grows without bound, so any caller reading
        it every turn should pass one.
        """
        ...
        with connect(self.db_path) as connection:
            if limit is None:
                rows = connection.execute(
                    """
                    SELECT event_ts, turn_index, role, kind, name, content_json
    ...
    def state(
        self, session_cookie: str, history_limit: int | None = None
    ) -> dict[str, Any]:
        """Return model-visible session state and previously observed history.
        ...
        Slack data in this response is data already returned by tool calls in
        this session's history. Verifiers use the protected admin interface
        when they need authoritative world state.
        """
        session = self._get_session(session_cookie)
        return {
            "session": {
    ...
    def _ensure_session_schema(self) -> None:
        with connect(self.db_path) as connection:
            ...
            # Databases created by an earlier contract version predate these
            # columns; adding them here keeps an existing sandbox usable.
            existing = {
                row[1] for row in connection.execute("PRAGMA table_info(rl_sessions)")
            }
            for column, ddl in (
                ("max_turns", "INTEGER NOT NULL DEFAULT 100"),
                ("last_progress", "REAL NOT NULL DEFAULT 0.0"),
                ...
                if column not in existing:
                    connection.execute(f"ALTER TABLE rl_sessions ADD COLUMN {column} {ddl}")
            connection.commit()
    ...
    def _get_session(self, session_cookie: str) -> sqlite3.Row:
        ...
        with connect(self.db_path) as connection:
            session = connection.execute(
                "SELECT * FROM rl_sessions WHERE session_cookie = ?",
                (session_cookie,),
            ).fetchone()
        if session is None:
    ...
    def _append_history(
        ...
        # Transcript bookkeeping observes world time without moving it; the
        # row's position in the conversation comes from event_id, not from ts.
        # Ticking here is what used to let read-only tool calls advance Slack's
        # wall clock and perturb the timestamps of later mutations.
        event_ts = VIRTUAL_CLOCK.now(connection)
        connection.execute(
            """
            INSERT INTO conversation_history (
                event_ts, session_cookie, turn_index, role, kind, name, content_json
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (event_ts, session_cookie, turn, role, kind, name, json.dumps(content, sort_keys=True)),
        )
