...
class SubprocessBackend(ToolBackend):
    """Drive the ``slack`` CLI as a child process.
...
class HarborBackend(ToolBackend):
    """Run the ``slack`` CLI inside a Harbor-managed environment.
    ...
    def __init__(self, environment: Any, *, executable: str = "slack", timeout_sec: int = 120) -> None:
...
class InProcessBackend(ToolBackend):
    ...
    async def call(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        from slack_sim.server import _from_error
        from slack_sim.service import execute_tool
        from slack_sim.sqlite_common import WorldError
