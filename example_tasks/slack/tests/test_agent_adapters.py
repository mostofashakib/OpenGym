...
from slack_sim.service import export_state, seed_database
        ...
        completion = provider.complete(
            ...
            messages=[
                {"role": "user", "content": "do the thing"},
                {
                    "role": "assistant",
                    "content": "looking",
                    "tool_calls": [ToolCall("c1", "list_channels", {"cursor": "p2"})],
                },
                {"role": "tool", "tool_call_id": "c1", "name": "list_channels", "content": "{}"},
            ],
            tools=TOOLS,
        )
    finally:
...
OLLAMA_REPLY = {
    "message": {
        "content": json.dumps(
            {
                "thought": "on it",
                "actions": [{"tool": "list_channels", "arguments": {"cursor": "p2"}}],
                "answer": "",
            }
        )
    },
...
ANTHROPIC_REPLY = {
    "content": [
        ...
        {"type": "tool_use", "id": "toolu_a", "name": "list_channels", "input": {"cursor": "p2"}},
    ],
...
def test_every_provider_decodes_the_same_turn_identically() -> None:
    ...
    for provider, reply, expected_id in cases:
        ...
        check(f"{provider.name}: name", call.name == "list_channels", call.name)
        check(f"{provider.name}: arguments", call.arguments == {"cursor": "p2"}, str(call.arguments))
...
def test_each_provider_is_sent_its_own_dialect() -> None:
    ...
    check("the catalogue reaches the prompt",
          "list_channels" in ollama.payload["messages"][0]["content"])
    ...
    check("ollama replays its own action object", replayed["actions"][0]["tool"] == "list_channels",
          str(replayed))
    ...
    check("ollama is told which tool a result came from",
          ollama.payload["messages"][3].get("tool_name") == "list_channels")
    ...
    encoded = _encode_anthropic(
        [
            {"role": "user", "content": "go"},
            {
                ...
                "content": "",
                "tool_calls": [
                    ToolCall("a", "get_channel_messages", {"channel_id": "C019"}),
                    ToolCall("b", "get_channel_messages", {"channel_id": "C020"}),
                ],
            },
            {"role": "tool", "tool_call_id": "a", "name": "get_channel_messages", "content": "{}"},
            {"role": "tool", "tool_call_id": "b", "name": "get_channel_messages", "content": "{}"},
        ]
    )
...
def test_arguments_are_decoded_without_ever_raising() -> None:
    ...
    for raw, label in cases:
        decoded = _decode_arguments(raw, "list_channels")
...
def test_the_loop_stops_for_the_right_reasons() -> None:
    with Sandbox() as sandbox:
        answered = asyncio.run(
            ToolLoop(
                ScriptedProvider([_call("list_channels", {}), Completion(text="finished")]),
                sandbox.backend,
                system="s",
            ).run("go")
        )
        ...
        capped = asyncio.run(
            ToolLoop(
                ScriptedProvider([_call("get_channel_messages", {"channel_id": f"C0{i:02d}"}) for i in range(10)]),
                sandbox.backend,
                system="s",
                max_turns=3,
            ).run("go")
        ...
        stuck = asyncio.run(
            ToolLoop(
                ScriptedProvider([_call("list_channels", {}) for _ in range(10)]),
                sandbox.backend,
                system="s",
                max_turns=20,
                repeat_limit=3,
        ...
        varied = asyncio.run(
            ToolLoop(
                ScriptedProvider([_call("get_channel_messages", {"channel_id": f"C0{i:02d}"}) for i in range(6)]),
                sandbox.backend,
                system="s",
                max_turns=20,
                repeat_limit=3,
        ...
        broken = asyncio.run(
            ToolLoop(
                ScriptedProvider([_call("list_channels", {})]), BrokenBackend(), system="s"
            ).run("go")
        )
...
def test_a_refusal_reaches_the_model_intact() -> None:
    with Sandbox() as sandbox:
        provider = ScriptedProvider([_call("get_channel_messages", {"channel_id": "C024"}), Completion(text="ok")])
        ...
        check("the code reaches the model", "permission_denied" in shown, shown[:200])
        check("so does the message", "not a member" in shown.lower(), shown[:200])
...
def test_a_long_result_is_truncated_and_says_so() -> None:
    with Sandbox() as sandbox:
        provider = ScriptedProvider([_call("list_channels", {}), Completion(text="ok")])
        asyncio.run(
        ...
        # A channel read is not that -- this workspace's threads run to several
        # kilobytes, which is why the truncation exists at all.
        short = ScriptedProvider([_call("join_channel", {"channel_id": "C011"}), Completion(text="ok")])
...
def test_the_system_prompt_carries_the_role_and_the_rules() -> None:
    prompt = system_prompt("incident_coordinator")
    check("the role is present", "incident" in prompt.lower(), prompt[:80])
...
class Sandbox:
    ...
    def reward(self) -> float:
        ...
        evaluation = TieredRewardEngine.for_preset(build_contract(), None).evaluate(
            Episode.from_state(self.state())
        )
        return float(evaluation.reward)


# Two real fragments of the reference episode rather than the whole of it:
# `dynamic_scenario.drive_terminal` resolves thread timestamps from live state
# mid-sequence, so it cannot be flattened into a fixed script. These are enough
# to show that reward tracks what the loop left in the world.
JOINED_THE_BRIDGE = [
    _call("search_channels", {"query": "cutover"}),
    _call("join_channel", {"channel_id": "C024"}),
    _call("get_channel_messages", {"channel_id": "C024"}),
    Completion(text="Joined the cutover bridge."),
]

REVIEWED_THE_SAML_CHANGE = [
    _call("search_messages", {"query": "IDP-ACME-014"}),
    _call(
        "reply_to_thread",
        {
            "thread_parent_id": "MSG194",
            "body": "Critique: `includes` is substring matching, but the SAML audience "
            "must match by exact equality. Use === after canonicalization.",
        },
    ),
    _call("get_channel_messages", {"channel_id": "C023"}),
    _call("reply_to_thread", {"thread_parent_id": "MSG194", "body": "Looks good to me"}),
    Completion(text="Reviewed the audience check."),
]

BOTH = [*JOINED_THE_BRIDGE[:-1], *REVIEWED_THE_SAML_CHANGE]


def test_the_loop_actually_changes_the_world_it_is_graded_on() -> None:
    with Sandbox() as sandbox:
        result = asyncio.run(
            ToolLoop(ScriptedProvider(JOINED_THE_BRIDGE), sandbox.backend,
                     system="s", max_turns=20).run("go")
        )
        ...
        # The bridge is not in the seeded membership, so this is a state change
        # only the loop could have made.
        joined = {
            row["channel_id"] for row in sandbox.state()["memberships"]
            if row["user_id"] == "U002"
        }
        check("the loop joined the cutover bridge", "C024" in joined, str(sorted(joined))[:200])
        bridge_reward = sandbox.reward()
        check("and it is paid for", bridge_reward > 0.0, f"{bridge_reward:.4f}")

    # The false positive this guards against: a harness that pays for running
    # rather than for what the run left behind.
    with Sandbox() as sandbox:
        asyncio.run(
            ToolLoop(ScriptedProvider([Completion(text="I would rather not.")]),
                     sandbox.backend, system="s").run("go")
        )
        check("abstaining scores exactly zero", sandbox.reward() == 0.0, f"{sandbox.reward():.4f}")

    with Sandbox() as sandbox:
        asyncio.run(
            ToolLoop(ScriptedProvider(BOTH), sandbox.backend, system="s", max_turns=20).run("go")
        )
        both_reward = sandbox.reward()
        check("doing more of the work pays more", both_reward > bridge_reward,
              f"{both_reward:.4f} vs {bridge_reward:.4f}")
        check("and a partial episode is still short of the full score", both_reward < 1.0,
              f"{both_reward:.4f}")
...
def test_the_action_schema_can_only_describe_real_tools() -> None:
    ...
    for payload, why in (
        ({"tool": "get_channel_messages", "arguments": {"channel_id": "C019"}}, None),
        ({"tool": "delete_everything", "arguments": {}}, "an invented tool"),
        ({"tool": "get_channel_messages", "arguments": {"channel_id": 19}}, "a wrongly typed argument"),
        ({"tool": "get_channel_messages", "arguments": {"channel_id": "C019", "limit": "ten"}},
         "a string where an integer is declared"),
        ({"tool": "get_channel_messages", "arguments": {"nonsense": "x"}},
         "an argument that does not exist"),
        ({"tool": "get_channel_messages", "arguments": {}}, "a missing required argument"),
        ({"tool": "get_channel_messages"}, "a missing arguments object"),
        ({"tool": "invite_to_channel", "arguments": {"channel_id": "C019", "user_ids": "U003"}},
         "a bare string where a list is declared"),
    ):
        rejected = True
        try:
...
def test_the_catalogue_says_what_the_schema_cannot() -> None:
    ...
    signature = catalogue.split("get_channel_messages(")[1].split(")")[0]
    check("optional arguments are marked", "cursor?" in signature, signature)
    check("required ones are not", "channel_id:" in signature, signature)
    check("declared types come along", "limit?: integer" in signature, signature)
    check("so do list types", "[string]" in catalogue, catalogue[:400])
    check("and the descriptions", "Join a public channel" in catalogue)


def test_strict_mode_normalisation_keeps_the_same_tool() -> None:
    messages = next(t for t in REAL_TOOLS if t["name"] == "get_channel_messages")
    strict = strict_parameters(messages["input_schema"])
    original = messages["input_schema"]
    ...
    check("a required argument keeps its type",
          strict["properties"]["channel_id"]["type"] == "string")
    check("an optional one becomes nullable",
          strict["properties"]["cursor"]["type"] == ["string", "null"])
    check("including a non-string one",
          strict["properties"]["limit"]["type"] == ["integer", "null"])
    # And the reason it is safe to do that: the nulls never reach the world,
    # which would read a null as a value rather than as an omission.
    cleaned = drop_nulls({"channel_id": "C019", "cursor": None, "limit": None})
    check("nulls are dropped before dispatch", cleaned == {"channel_id": "C019"}, str(cleaned))
    check("but a legitimate false is not", drop_nulls({"flag": False}) == {"flag": False})
    check("and neither is an empty list", drop_nulls({"user_ids": []}) == {"user_ids": []})
    check("nor a zero", drop_nulls({"limit": 0}) == {"limit": 0})


def test_the_validator_enforces_the_schema_the_model_was_shown() -> None:
    """What the client must never do is refuse a well-formed call.

    It is allowed to refuse a malformed one the workspace would have tolerated,
    and here it does: this Slack surface does not enforce required arguments at
    all, so `search_messages` with no query returns a result rather than a
    refusal. The schema says `query` is required, the model was constrained
    against that schema, and a call omitting it is a model error worth naming
    rather than a search of everything. Being stricter than the world in that
    direction is the point; being stricter about a *valid* call would be a bug,
    and that is what the first group below pins.

    Checked against the real tool surface rather than a second validator, so
    "what the world would accept" is not a guess about it.
    """
    schemas = {tool["name"]: tool["input_schema"] for tool in REAL_TOOLS}
    ...
    # Well-formed calls, each exercising a different corner of the schema
    # vocabulary: required only, optional added, an integer, and a list.
    accepted = [
        ("list_channels", {}),
        ("list_channels", {"limit": 5}),
        ("get_channel_messages", {"channel_id": "C019"}),
        ("get_channel_messages", {"channel_id": "C019", "limit": 3}),
        ("search_messages", {"query": "IDP-ACME-014"}),
        ("join_channel", {"channel_id": "C024"}),
        ("reply_to_thread", {"thread_parent_id": "MSG194", "body": "Looks good to me"}),
    ]
    with Sandbox() as sandbox:
        for name, payload in accepted:
            envelope = asyncio.run(sandbox.backend.call(name, payload))
            world_ok = bool(envelope.get("ok"))
            check(
                f"the world accepts {name}{payload}", world_ok,
                str(envelope.get("error"))[:160],
            )
            check(
                f"and so does the client: {name}{payload}",
                not (world_ok and not client_accepts(name, payload)),
                "the client refused a call the workspace honoured",
            )

    # Malformed against the published schema. The client refuses every one, and
    # names the field. The workspace mostly refuses too, but it is answering a
    # different question -- it discovers the mistake downstream, as a lookup
    # that failed, so the reason it gives misdiagnoses the cause. Passing a
    # bare string where a list of user ids belongs comes back as "User U was
    # not found", because the string was iterated character by character. A
    # model told that will go looking for a user; a model told its argument
    # should have been a list will fix the call.
    with Sandbox() as sandbox:
        for name, payload, world_refuses_too in (
            ("get_channel_messages", {"channelid": "C019"}, True),
            ("get_channel_messages", {"channel_id": 19}, True),
            ("get_channel_messages", {"channel_id": "C019", "limit": "ten"}, True),
            ("invite_to_channel", {"channel_id": "C019", "user_ids": "U003"}, True),
            # And one the workspace answers rather than refuses: this surface
            # does not enforce required arguments, so a search with no query is
            # a search of everything. Here the client is the only guard.
            ("search_messages", {}, False),
        ):
            check(f"the client refuses {name}{payload}", not client_accepts(name, payload),
                  str(payload))
            envelope = asyncio.run(sandbox.backend.call(name, payload))
            check(
                f"the workspace {'refuses it too' if world_refuses_too else 'would have answered it'}",
                (not envelope.get("ok")) is world_refuses_too,
                str(envelope)[:160],
            )
...
def test_a_call_the_schema_does_not_describe_never_reaches_the_world() -> None:
    with Sandbox() as sandbox:
        provider = ScriptedProvider([
            _call("obliterate_channel", {"channel_id": "C019"}),
            _call("get_channel_messages", {"channel_id": "C019", "limit": "ten"}),
            _call("join_channel", {"channel_id": "C011"}),
        ...
        check("with the real ones listed", "join_channel" in first["error"]["message"])
        check("a wrongly typed argument is refused", second["ok"] is False, str(second))
        check("as a schema violation", second["error"]["code"] == "schema_violation", str(second))
        check("naming the field", "limit" in second["error"]["message"], str(second))
    ...
    with Sandbox() as sandbox:
        broken = Completion(
            tool_calls=(
                ToolCall("c1", "join_channel", _decode_arguments("{oops", "join_channel")),
            )
        )
        result = asyncio.run(
...
def main() -> int:
    ...
    for test in (
        ...
        test_the_validator_enforces_the_schema_the_model_was_shown,
