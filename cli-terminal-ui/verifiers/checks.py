"""State checks for terminal environment verification."""
...
def _load_state(client: Any | None = None, state_data: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    # 1. Try sockets
    for sock in [
        os.environ.get("TERMINAL_ADMIN_SOCKET", "/run/terminal/admin.sock"),
        os.environ.get("TERMINAL_SOCKET", "/run/terminal/agent.sock"),
    ]:
    ...
    for candidate in [
        Path("/var/lib/terminal/state-export.json"),
    ...
    if state is None:
        return {}, [
            CheckOutcome(
                ...
                passed=False,
                detail={"error": "Could not retrieve terminal state from socket, artifact, or database"},
                weight=1.0,
            )
        ]

    eval_dict = evaluate_terminal_episode(state)
