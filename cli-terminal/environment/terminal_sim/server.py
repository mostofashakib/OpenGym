"""Unix socket server daemon for terminal environment."""
...
from terminal_sim.protocol import recv_json, send_json
...
def run_daemon(
    db_path: Path | str = "/var/lib/terminal/terminal.db",
    agent_socket: str = "/run/terminal/agent.sock",
    admin_socket: str = "/run/terminal/admin.sock",
) -> None:
    ...
    print(f"Terminal daemon started on {agent_socket} and {admin_socket}")
    try:
        agent_srv.start()
...
if __name__ == "__main__":
    db = os.environ.get("TERMINAL_DB", "/var/lib/terminal/terminal.db")
    agent_sock = os.environ.get("TERMINAL_SOCKET", "/run/terminal/agent.sock")
    admin_sock = os.environ.get("TERMINAL_ADMIN_SOCKET", "/run/terminal/admin.sock")
