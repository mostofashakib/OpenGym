...
if [ -d "/tests" ]; then
  ...
  SUITES=(test_mcp_server test_verifiers test_browser_sim test_rl_env test_end_to_end)
else
  ...
  SUITES=(test_agent_adapters test_mcp_server test_verifiers test_browser_sim test_rl_env test_end_to_end)
fi
