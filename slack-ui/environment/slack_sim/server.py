"""The Slack world server: the only way in from outside the environment.

This process owns the workspace database. It runs in the environment's own
container as the ``slackd`` user, and nothing outside that container has a
...
``agent.sock`` (0666)
    The Slack tool surface, nothing else. This is what the agent is given.
...
The split is by socket rather than by a flag on one endpoint so that reaching
an admin operation from the agent's socket is not a check that could be
bypassed -- the dispatch table simply does not contain it.
"""
...
from slack_sim.protocol import ADMIN_SOCKET, AGENT_SOCKET, ProtocolError, decode, encode
...
from slack_sim.sqlite_common import (
    InternalError, UnknownToolError, WorldError, storage_errors,
)
from slack_sim.identity import LOGGED_IN_USER

LOGGER = logging.getLogger("slackd")
...
class WorldService:
    ...
    def call_tool(self, tool_name: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            # Every deliberate failure of the world is a WorldError and carries
            # its own code, so one clause reports a rule violation, a corrupt
    ...
    # -- admin surface ----------------------------------------------------
    # The admin ops read and rebuild the database directly, so they meet the
    # same storage faults the tool surface does and are given the same name for
    # them. A grader told `storage_error` knows the export is untrustworthy;
    # one told `internal_error` cannot tell that from a bug in the exporter.
...
def _from_error(exc: WorldError) -> dict[str, Any]:
    ...
    Reading the code off the exception rather than off the catch site is what
    keeps the taxonomy from drifting: a new subclass is reportable the moment
    it exists, and no handler can quietly relabel one kind of failure as
    another.
    """
...
def _dispatch(world: WorldService, allowed: tuple[str, ...], request: dict[str, Any]) -> dict[str, Any]:
...
class _Server(socketserver.ThreadingUnixStreamServer):
    ...
    def __init__(self, path: str, world: WorldService, allowed_ops: tuple[str, ...], mode: int) -> None:
...
def serve(
    ...
    # when the file happens to be missing: an episode must never inherit
    # another episode's messages or half-fired scenario events, and relying on
    # container freshness for that makes it an accident rather than a rule.
    world.seed()
    ...
    # 0600 and owned by the server's uid (root): the unprivileged agent user
    # cannot open this at all.
    admin = _Server(admin_socket, world, ADMIN_OPS, 0o600)
    LOGGER.info("slackd listening on %s (agent) and %s (admin)", agent_socket, admin_socket)
    threading.Thread(target=admin.serve_forever, daemon=True).start()
    agent.serve_forever()


def main() -> None:
    parser = argparse.ArgumentParser(description="Slack world server.")
